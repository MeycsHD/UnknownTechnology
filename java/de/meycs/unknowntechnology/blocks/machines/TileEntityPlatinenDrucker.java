...
public class TileEntityPlatinenDrucker extends TileEntity implements IInventory, ITickable {
	...
	public static boolean isBurning(BlockPlatinenDrucker te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			BlockPlatinenDrucker.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = PlatinenDruckerRecipe.getInstance().getSinteringResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = PlatinenDruckerRecipe.getInstance().getSinteringResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
	...
	public boolean isUsableByPlayer2(EntityPlayer player) 
	{
